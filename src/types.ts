...
export interface Problem {
  ...
  /** 자에 눈금을 몇 칸까지 그릴지 */
  rulerSpan: number;
  /** 자 위에 올릴 물건. 매번 달라야 "자 위의 또 다른 자"처럼 보이지 않는다 */
  rulerObject?: ObjectKind;
  /**
   * 틈을 이만큼의 **똑같은 칸**으로 나눠 보여준다. "12칸을 똑같은 조각 3개로"에 쓴다.
   *
   * 예전에는 조각 하나를 누르면 정해진 개수가 한꺼번에 놓이게 했는데,
   * **한 번 눌러서 답이 되니 생각할 것이 없었다.** 지금은 칸을 나눠 보여주고
   * 한 칸에 딱 맞는 조각만 들어가게 한다 — 12칸이 세 칸으로 나뉜 걸 보고
   * 그 한 칸에 맞는 조각을 찾는 것이 곧 나눗셈이다.
   */
  slots?: number;
